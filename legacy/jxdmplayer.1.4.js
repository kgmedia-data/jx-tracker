    ...
    window.jxdm = {
    	...
    	*/
    	jxstartDM: function(p){
    		...
    		if (p.adparameters.topbanner) this.topBannerTag = p.adparameters.topbanner;
    		if (p.adparameters.bottombanner) this.bottomBannerTag = p.adparameters.bottombanner;
    		if (p.adparameters.frametag) this.frameTag = p.adparameters.frametag;
    		if (p.adparameters.hotspotstag) this.hotspotTag = p.adparameters.hotspotstag;
    		if (p.adparameters.network == 'mobile') this.wifi = false; else this.wifi = true;
    		if (p.callbackready) this.callbackReady = p.callbackready;
    		if (p.adparameters) this.adparameters = p.adparameters; else this.adparameters = {};
    		if (p.visible) this.v = p.visible;
    		if (p.pagetitle) this.pagetitle = p.pagetitle.toLowerCase();
    		if (this.pagetitle && p.adparameters.excludewords !== undefined && p.adparameters.excludewords && p.adparameters.excludewords.length > 0){
    			...
    			this.pagetitle = this.pagetitle.replace(/ +(?= )/g,'').trim();
    			//console.log("New pagetitle is: " + this.pagetitle);
    		}

    		if (this.tdiv && this.adparameters){
		        ...
		        this.dm = jxutil.newDiv(this.tdiv, 'div', '', 'container');
			    ...
			    this.pparams = {
                	'controls': this.adparameters.controls || true,
        		...
        		if (this.adparameters.syndication) this.pparams.syndication = this.adparameters.syndication;

        		// Setting the autoplay. We autoplay only if set to wifi and we detected we are on a wifi network
        		if (this.wifi && this.adparameters.autoPlay == "wifi"){
        			this.auto = true;
        			this.pparams['autoplay-mute'] = true;
        			//this.pparams.mute = true;
        			//this.pparams.muted = true;
        		}else{
	        		this.auto = false;
	        		this.pparams.autoplay = false;        			
        		}

        		// We create a Dailymotion player instance
        		//console.log("Params for the initialisation of the player: " + JSON.stringify(this.pparams));

			    // Starting to play videos: try to call the search is any
			    if (this.adparameters.search && this.adparameters.search == 'title' && this.pagetitle){
			        ...
			        if (this.adparameters.defaultPlaylist){
			        	this.apiplaylist(this);
	        			window.addEventListener('onSearchEnd', (e) => { window.jxdm.createPlayer(window.jxdm); });
	                }else if(this.adparameters.defaultVideo){
                        this.videolist.push({id: this.adparameters.defaultVideo}); // we add the default video to the list of videos to play
		...
		jxMsgIn: function(e){
			try{
				if (e.data == 'jxvisible') {
			    	//console.log("We received a VISIBLE message");
			        window.jxdm.v = true; // setting the visibility
		        	if (!window.jxdm.view){jxutil.fire('creativeView');window.jxdm.view = true;}
		            if (!window.jxdm.l){
			    		//console.log("No video loaded, we load a video");
		            	window.jxdm.LoadNextVideo(); // if there is no video already loaded then we load the next video
		            }
		            else if (!window.jxdm.p && window.jxdm.adparameters.autoPlay != "none" && window.jxdm.r){
		            	//console.log("No video plays, whereas it is autoplay and the player is ready.");
		            	if (window.jxdm.wifi && window.jxdm.adparameters.autoPlay == 'wifi'){
		            		//console.log("We are in Wifi, so we force autoplay.");
		            		window.jxdm.player.play();
		            		window.jxdm.p = true;
		            	}else{
			    	...
			    	//console.log("We received a not visible message, we PAUSE the video");
			        window.jxdm.v = false;
			        if (window.jxdm.p){
			        	window.jxdm.player.pause(); // a video is playing, then we pause the video
			        	window.jxdm.p = false;
			        }
			    }				
			}catch (e){
