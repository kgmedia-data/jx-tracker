		...
		*/
		...
		/*
		window.jxosmevtc_={push:function(t){let o={type:"string"==typeof t?t:t.action?t.action:"click"};"object"==typeof t&&(o.params=t);let n="jxmsg::"+JSON.stringify(o);(window.location!==window.parent.location?parent:window).postMessage(n,"*")}};
		window.jxosmevtc_ = { 
			push: function(o) {
				let obj = {
					type: (typeof o === 'string' ? o:(o.action? o.action:'click'))
				};
				if (typeof o === 'object') {
					obj.params = o;
				}
				let msgStr = "jxmsg::" + JSON.stringify(obj);
				let tgt = ( window.location !== window.parent.location ? parent: window);
				tgt.postMessage(msgStr, "*");
			}
		};
		jxosmevtc_.push({ action: 'jxsuppress'});
		jxosmevtc_.push({ id: 'link2' });
		jxosmevtc_.push({ id: 'link2', url: 'https://www.zaobao.com' });
		*/
