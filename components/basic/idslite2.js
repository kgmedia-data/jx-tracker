...
function makeIdsO_(namedCookie = null) {
    let ret = {};
    try {
        let client_id   = null;
        let sid         = null;
        ...
        if (sid) { //it was not clear from Vincent's stuff, but I guess we will have to write this thing 
            // back then.
            ls.setItem(jxSidName_, sid);
            writeC([{n:jxSidName_, v:sid, d: new Date(new Date().setMinutes(new Date().getMinutes() + sidTTLMins_))}]);
        }
...
module.exports.get = makeIdsO_;

/* 
 ************** module: basic/idslite2 **************************************************
