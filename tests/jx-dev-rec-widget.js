...
(function() {
    ...
    function appendDefaultCSS(rand, blockwidth) {
        ...
        const stylesArr = [
            ...
            "." + recColCls + "" + rand + "{position:relative;box-sizing:border-box;width:"+ blockwidth+ "px;flex:0 0 " + blockwidth + "px;max-width:" + blockwidth + "px;}",
        ].join("\n");
        ...
        if (items.length > 0) {
            items.slice(0, count).map(function(item, index) {
                ...
                * in case more than 1 widget is embedded on the page) */
                ...
                var newwrapper2 = createElement('div', null, null, null, null);
                newwrapper2.style.cssText = "display:flex;flex-direction:column;margin:5px;"
                
                var newwrapper = createElement('div', null, null, null, null);
                newwrapper.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;padding:0px 10px;"

                var unHideItemDiv = createElement('div', null, null, null, 'Unhide');
                unHideItemDiv.style.cssText = "cursor:pointer;color:black;border:1px solid black;border-radius:5px;padding:5px;display:none;";
                unHideItemDiv.onclick = function(e) {
                    e.target.style.display = 'none';
                    hideItemDiv.style.display = 'block';
                    jxRecHelper.unhide(index)
                }
                var hideItemDiv = createElement('div', null, null, null, 'Hide');
                hideItemDiv.style.cssText = "cursor:pointer;color:black;border:1px solid black;border-radius:5px;padding:5px;";
                hideItemDiv.onclick = function(e) {
                    e.target.style.display = 'none';
                    unHideItemDiv.style.display = 'block';
                    jxRecHelper.hidden(index)
                }
                var sharedDiv = createElement('div', null, null, null, 'Share');
                sharedDiv.style.cssText = "cursor:pointer;color:black;border:1px solid black;border-radius:5px;padding:5px;";
                sharedDiv.onclick = function() {
                    jxRecHelper.shared(index)
                }
                var bookmarkDiv = createElement('div', null, null, null, 'Bookmark');
                bookmarkDiv.style.cssText = "cursor:pointer;color:black;border:1px solid black;border-radius:5px;padding:5px;";
                bookmarkDiv.onclick = function() {
                    jxRecHelper.bookmarked(index)
                }
                newwrapper.appendChild(unHideItemDiv);
                newwrapper.appendChild(hideItemDiv);
                newwrapper.appendChild(sharedDiv);
                newwrapper.appendChild(bookmarkDiv);

                recItem.appendChild(imgWrapper);
                recItem.appendChild(categoryDiv);
                recItem.appendChild(titleDiv);

                newwrapper2.appendChild(newwrapper);
                newwrapper2.appendChild(recItem);
                widgetWrapper.appendChild(newwrapper2);
    ...
    const _jxRecSdkURL = "https://jixie-creative-debug.s3-ap-southeast-1.amazonaws.com/tests/jxrecsdk.1.0.min.js"
